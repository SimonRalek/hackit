class Jeep extends Vehicle {
    constructor(posx, posy) {
        ...
        this.spriteNormal.src = `../assets/poust/onRoad.png`;
        this.spriteNormal.width = 85;
        this.spriteNormal.height = 45;
        this.spriteOutside = new Image();
        this.spriteOutside.src = `../assets/poust/offRoad.png`;
        this.size = { x: this.spriteNormal.width, y: this.spriteNormal.height };
        this.pos = { x: posx - this.size.x / 2, y: posy - this.size.y / 2 };
    }
    draw() {
        if (!intersect(this.getPolygon(), map2BetterData).length == 0) {
            ctx.save();
